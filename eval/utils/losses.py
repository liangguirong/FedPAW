...
#     return mse_loss
def consistency_loss(logits_s, logits_w, mask):

    assert logits_s.size() == logits_w.size()
    pseudo_label = F.softmax(logits_w, dim=1)
    _, max_idx = torch.max(pseudo_label, dim=-1)
    log_pred = F.log_softmax(logits_s, dim=-1)
    masked_loss = F.nll_loss(log_pred, max_idx.long(), reduction='none') * mask.float()
    return masked_loss.mean()
def consistency_loss1(logits_s, logits_w):

    assert logits_s.size() == logits_w.size()
    pseudo_label = F.softmax(logits_w, dim=1)
    _, max_idx = torch.max(pseudo_label, dim=-1)
    log_pred = F.log_softmax(logits_s, dim=-1)
    masked_loss = F.nll_loss(log_pred, max_idx.long(), reduction='none')
